module marketplace_addr::marketplace {
    ...
    #[resource_group_member(group = aptos_framework::object::ObjectGroup)]
    struct FixedPriceListing<phantom AptosCoin> has key {
    ...
    public entry fun list_with_fixed_price<AptosCoin>(
        seller: &signer,
        object: object::Object<object::ObjectCore>,
        price: u64,
    ) acquires SellerListings, Sellers, MarketplaceSigner {
        list_with_fixed_price_internal<aptos_coin::AptosCoin>(seller, object, price);
    }

    /// Purchase outright an item from a fixed price listing.
    public entry fun purchase<AptosCoin>(
        purchaser: &signer,
        object: object::Object<object::ObjectCore>,
        ...
        assert!(exists<FixedPriceListing<aptos_coin::AptosCoin>>(listing_addr), error::not_found(ENO_LISTING));

        let FixedPriceListing {
            price,
        } = move_from<FixedPriceListing<aptos_coin::AptosCoin>>(listing_addr);

        // The listing has concluded, transfer the asset and delete the listing. Returns the seller
        // for depositing any profit.

        let coins = coin::withdraw<aptos_coin::AptosCoin>(purchaser, price);
    ...
    public(friend) fun list_with_fixed_price_internal<AptosCoin>(
        seller: &signer,
        object: object::Object<object::ObjectCore>,
        price: u64,
        ...
        let fixed_price_listing = FixedPriceListing<aptos_coin::AptosCoin> {
            price,
        };
    ...
    #[view]
    public fun price<AptosCoin>(
        object: object::Object<Listing>,
    ): option::Option<u64> acquires FixedPriceListing {
        let listing_addr = object::object_address(&object);
        if (exists<FixedPriceListing<aptos_coin::AptosCoin>>(listing_addr)) {
            let fixed_price = borrow_global<FixedPriceListing<aptos_coin::AptosCoin>>(listing_addr);
